...
#define DSIGMAAT(x) (0.5*(1.0 - (x/STRETCH)*(x/STRETCH))/STRETCH) //derivative of SIGMA at SIGMA^{-1}(x)
...
void initweights() {
  ...
  if ((weightpoint = fopen(WEIGHTFILE, "rb")) == NULL) {
    ...
    if(fread(weight, (NODES*MAX(MAX(INLAYER,HLAYER1),HLAYER2)) * sizeof(double), 1, weightpoint) != 1){
...
void saveweights() {
  ...
  if(fwrite(weight,(NODES*MAX(MAX(INLAYER,HLAYER1),HLAYER2))*sizeof(double), 1, weightpoint) != 1)
...
int search (depth, alpha, beta, top) {
  ...
  if (depth) {
    ...
    for(lr=0; lr<sidescheck; lr++) //left or right
      if (legalmove(i,j,lr,fval)) {
        ...
        else if ((top) && (value == localalpha) && (!(rand()%(++bestmoves)))) {
          savei = i;
          savej = j;
          savelr = lr;
          savevalue = value;
  ...
  else return evaluate(current)-evaluate(SW(current));
}
...
double quality(char update,double desired) {
  ...
  if(update) {
    ...
    for(j=INLAYER+HLAYER1; j<INLAYER+HLAYER1+HLAYER2; j++) {
      error[j] = error[INLAYER+NODES-1]*DSIGMAAT(output[INLAYER+NODES-1])*weight[NODES-1][PLACE2(j)];
    ...
    for(i=INLAYER+HLAYER1; i<INLAYER+HLAYER1+HLAYER2; i++)
      for(j=INLAYER; j<INLAYER+HLAYER1; j++) {
        ...
        for(k=HLAYER1; k<HLAYER1+HLAYER2; k++)
          error[j] += error[INLAYER+k]*DSIGMAAT(output[INLAYER+k])*weight[k][PLACE1(j)];
        for(k=HLAYER1; k<HLAYER1+HLAYER2; k++)
    ...
    for(i=INLAYER; i<INLAYER+HLAYER1; i++)
      for(j=0; j<INLAYER; j++) {
        error[j] = 0.0;
        for(k=0; k<HLAYER1; k++)
          error[j] += error[INLAYER+k]*DSIGMAAT(output[INLAYER+k])*weight[k][j];
        for(k=0; k<HLAYER1; k++)
          weight[k][j] -= CAP(ALPHA*error[j]);
      }
  }
  for(i=0;i<NODES;i++)
    printf("E%d=%lf, ", i,error[i+INLAYER]);
  printf("\n");
  for(i=0;i<NODES;i++)
    printf("W%d=%lf, ", i,weight[i][0]);
  printf("\n");
...
//   return 0;
  ...
  while ((search(1,LOSE,WIN,0) != LOSE) && (search(1,LOSE,WIN,0) != WIN)) {
    if (VERBOSE || HUMAN)
      printboard();
    ...
    if (current == FIRST)
      fprintf(outpoint,"%d. ",1+movenumber/2);
